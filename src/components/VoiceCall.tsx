import React, { useState, FormEvent, useEffect, forwardRef, useImperativeHandle, useCallback } from 'react';
import { useAuth } from '@/context/AuthContext';
import { useTwilioDevice } from '@/hooks/useTwilioDevice';
import { PhoneIcon, XCircleIcon } from '@heroicons/react/24/solid';
...
import CallPricing from '@/components/CallPricing';
...
const VoiceCall = forwardRef<VoiceCallHandle, VoiceCallProps>(
  ({ 
    title = "Phone", 
    userId = "", 
    hideHistory = false, 
    ...
    const [lastReportedRate, setLastReportedRate] = useState<number | null>(null);
    ...
    useEffect(() => {
      ...
      setBalanceFetchError(null);
      const userRef = doc(db, 'users', userId); // Use doc here

      const unsubscribe = onSnapshot(userRef, // Use onSnapshot here
        (docSnap) => { // Type docSnap if needed, but error is usually handled below
          if (docSnap.exists()) {
            ...
            console.warn(`[VoiceCall] User document snapshot not found for UID: ${userId}. Setting balance to 0.`);
            setUserBalance(0); 
            setBalanceFetchError('User profile not found.');
          }
          setIsLoadingBalance(false); 
        },
        (error: unknown) => { // Type error parameter
    ...
    useEffect(() => {
      ...
      if (currentCallFromHook && !isConnected && !isConnecting) {
        console.log('Incoming call detected but will be automatically rejected by hook logic');
      }
    }, [currentCallFromHook, isConnected, isConnecting]);
    ...
    useEffect(() => {
      if (isConnected) {
        ...
        // If connection drops and we had a start time, consider call ended
        if (callStartTime) {
          console.log('Call disconnected.');
          // Call hangup logic *without* recording cost again if already handled
          // handleHangup(false); // Let handleHangup decide based on currentCall state
          // Reset timer immediately on disconnect
          if (timerIntervalRef) {
          ...
          setCurrentCall(null); // Clear local call state
        }
      }
      // Add timerIntervalRef to dependencies
    }, [isConnected, callStartTime, timerIntervalRef]); 

    // Timer effect
    useEffect(() => {
      if (isConnected && callStartTime) {
        if (timerIntervalRef) clearInterval(timerIntervalRef); // Clear previous interval
    ...
    const handlePricingUpdate = (price: number, cost: number) => {
      setLastReportedRate(price);
      // Update state for real-time monitoring
      setCurrentCallCost(cost); 
    };
    
    // --- Define handleHangup BEFORE the useEffect that uses it --- 
    const handleHangup = useCallback(async (recordCost = true) => {
      // Remove hook call from here
      // const { device } = useTwilioDevice({ userId }); 
      
      if (!currentCall) {
        console.log("handleHangup called but no currentCall.");
        return;
      }
      console.log("Hanging up call...");
      
      const callSid = currentCall.parameters.CallSid;
      const callStartTimeForEnd = callStartTime; 
      const finalRate = lastReportedRate; 
      ...
      if (recordCost && callStartTimeForEnd !== null && finalRate !== null && finalRate >= 0) {
          const finalCost = currentCallCost; 
          console.log(`Final duration: ${finalDuration}s, Final rate: ${finalRate}, Final calculated cost: ${finalCost}`);
  
          if (!userId) {
            console.error("Cannot record call cost: User ID is missing.");
            setError("Could not record call cost due to missing user ID.");
            // Don't return here, still try to disconnect device below
          } else {
                try {
                  console.log(`Recording call cost for SID: ${callSid}, User: ${userId}`);
                  ...
                  const response = await fetch('/api/call-cost', {
                    ...
                    body: JSON.stringify({
                      id: callSid || `local-${Date.now()}`,
                      phoneNumber: validatedE164Number, 
                      timestamp: callStartTimeForEnd,
                  ...
                  if (!response.ok) {
                    if (data.error?.includes('Insufficient funds')) {
                        console.warn("Call ended due to insufficient funds (reported by API).");
                        // Don't display this specific error to the user, it's handled by hangup
                    } else {
                        throw new Error(data.error || 'Failed to record call cost');
                    }
                  } else {
                    console.log("Call cost recorded successfully by API.");
                  }
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                } catch (error: any) {
                  console.error("Error recording call cost:", error);
                  setError(error.message || "Failed to record call cost properly.");
                }
          }
      } else if (!recordCost) {
          console.log("Hangup requested without recording cost.");
      } else {
          console.warn("Could not record call cost due to missing start time or rate.");
      }
      
      // Ensure device disconnects
      if (device) {
          console.log("[handleHangup] Calling device.disconnectAll()");
          device.disconnectAll(); 
      } else {
          console.warn("[handleHangup] Cannot disconnect, Twilio device not available.");
      }
    // Update dependencies - include timerIntervalRef
    }, [currentCall, callStartTime, lastReportedRate, timer, userId, validatedE164Number, currentCallCost, user, setTimerIntervalRef, setError, setCurrentCall, setCallStartTime, setTimer, device, timerIntervalRef]);

    // --- Now add the balance monitoring useEffect AFTER handleHangup is defined --- 
    useEffect(() => {
    ...
    // Update local currentCall state when the hook's call state changes
    useEffect(() => {
        setCurrentCall(currentCallFromHook); 
    }, [currentCallFromHook]);
        ...
        <div className="p-4">
          {hookError && !isReady && (
            <div className="mb-4 p-3 bg-red-100 border border-red-300 text-red-800 rounded-md text-sm">
              ...
              <p>{hookError}</p>
              {hookError.includes('Microphone access') && (
                <div className="mt-3">
              ...
              {hookError.includes('no longer valid') && (
                <div className="mt-3">
          ...
          {/* Call UI */}
          {(isReady || currentCall || isConnecting || isConnected) && !hookError && (
            <>
              {showHistory ? (
                <CallHistory 
                  calls={callHistory} 
